...
(function () {
  ...
  function register() {
    ...
    DocumentPip.prototype._ensureHijack = function () {
      var self = this;
      this._hijackNativePip();

      // Bounded poll: ~ every 250ms for up to 15s, until a video is hijacked.
      if (!this._pollTimer) {
        var attempts = 0;
        this._pollTimer = setInterval(function () {
          attempts++;
          self._hijackNativePip();
          var v = self._getVideo();
      ...
      // Observe the player root (or document) so a swapped-in video gets re-hijacked.
      if (!this._observer && typeof MutationObserver !== 'undefined') {
        var target = this._playerRoot || document.body;
        if (target) {
          this._observer = new MutationObserver(function () { self._hijackNativePip(); });
    ...
    // Resolve the FULL player wrapper (.playkit-player), not the inner video-only
    // container. In Kaltura Player V7, getView() returns .playkit-container, which holds
    // only the <video> engine — the control bar AND overlay plugins (e.g. the watermark)
    // render in the .playkit-player wrapper one level up. We must move that wrapper so the
    // controls and watermark travel into the PiP window with the video.
    DocumentPip.prototype._resolvePlayerRoot = function () {
      ...
      if (video && video.closest) {
        ...
        if (wrapper) {
          // If nested players exist, prefer the outermost .playkit-player.
          var outer = wrapper;
      ...
      if (typeof this.player.getView === 'function') {
        ...
        if (view) {
          if (view.closest) {
            var w2 = view.closest('.playkit-player');
            if (w2) return w2;
          }
          // getView() itself may be the container; use its parent wrapper if present.
